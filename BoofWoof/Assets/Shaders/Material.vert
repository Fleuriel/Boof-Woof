...
void main() {
    fragNormal = normalize(mat3(transpose(inverse(model))) * aNormal);  // Transform normal to world space
    fragWorldPos = vec3(model * vec4(aPos, 1.0));  // Transform position to world space
