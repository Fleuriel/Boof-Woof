...
public:
    UIComponent() {};
    UIComponent(int tid, glm::vec2 pos, glm::vec2 s, float layer, bool selectable, float opacity)
        : textureid(tid), position(pos), scale(s), layer(layer), selectable(selectable), UI_opacity(opacity) {
    };
    ~UIComponent() {};
